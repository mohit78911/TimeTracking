...
export default function AlertDialog({ handleClose, handleClickOpen, open }) {
  ...
  const deleteDataHandler = (id) => {
    const updateList =  localData.filter((val) => val !== id);
    setLocalData(updateList)
  };
  ...
  return (
    <div>
      <div className="tableoftask">
        {localData.map((val, i) => {
          return (
            <div key={i} className="table tabledata">
              <h2>
                {i + 1} {val}
              </h2>
              <button onClick={() => deleteDataHandler(val.i)}>Delete</button>
            </div>
          );
        })}
      </div>
